...
def call(body) {
    ...
    def CFG = [
        ...
        'deploy.path': config.'deploy.path' ?: '/opt/ozkan-prod',
        'deploy.backup_path': config.'deploy.backup_path' ?: '/opt/backups/ozkan-prod',
    ...
    pipeline {
        ...
        stages {
            stage('Checkout') {
                steps {
                    script {
                        echo "═══════════════════════════════════════"
                        echo "OZKAN PRODUCTION PIPELINE - Starting"
                        echo "═══════════════════════════════════════"

                        def modulePath = 'resources/com/ozkan/pipeline/modules/Checkout/ProdCheckout.groovy'
            ...
            stage('Build') {
                steps {
                    script {
                        def modulePath = 'resources/com/ozkan/pipeline/modules/Build/ProdBuild.groovy'
                        def moduleCode = libraryResource(modulePath)
                        evaluate(moduleCode)
                    }
                }
            ...
            stage('Test') {
                steps {
                    script {
                        def modulePath = 'resources/com/ozkan/pipeline/modules/Test/ProdTest.groovy'
                        def moduleCode = libraryResource(modulePath)
                        evaluate(moduleCode)
                    }
                }
            ...
            stage('Deploy') {
                steps {
                    script {
                        def modulePath = 'resources/com/ozkan/pipeline/modules/Deploy/ProdDeploy.groovy'
                        def moduleCode = libraryResource(modulePath)
                        evaluate(moduleCode)
                    }
                }
            ...
            stage('Smoke Test') {
                steps {
                    script {
                        def modulePath = 'resources/com/ozkan/pipeline/modules/SmokeTest/ProdSmokeTest.groovy'
                        def moduleCode = libraryResource(modulePath)
                        evaluate(moduleCode)
                    }
                }
            ...
            stage('Post-Deploy Validation') {
                steps {
                    script {
                        def modulePath = 'resources/com/ozkan/pipeline/modules/PostDeployValidation/ProdValidation.groovy'
                        def moduleCode = libraryResource(modulePath)
                        evaluate(moduleCode)
                    }
                }
        ...
        post {
            success {
                echo ""
                echo "═══════════════════════════════════════"
                echo "✓ OZKAN PRODUCTION PIPELINE - SUCCESS"
                echo "═══════════════════════════════════════"
            ...
            failure {
                echo ""
                echo "═══════════════════════════════════════"
                echo "✗ OZKAN PRODUCTION PIPELINE - FAILED"
                echo "═══════════════════════════════════════"
                echo "Build: #${env.BUILD_NUMBER}"
